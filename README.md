...
👋 Hi, I’m Fahmi — a physician working with clinical datasets, REDCap exports, and real-world data entry realities.

This repository is a **cleaned, modular re-implementation** of a real clinical data workflow:
from **messy REDCap exports** → **analysis-ready datasets** → **quality control checks**.

It reflects how clinical data *actually* looks in practice:
inconsistent units, repeat instruments, checkbox chaos, missing values, and human creativity 😅
...
│ ├── 03_calibration_rules.R # Yes/No, numeric, checkbox sanity
│ ├── 04_feature_extraction.R # Diagnosis logic, CNS classification
│ └── 05_qc_data_quality.R # Missingness & range QC
│
├── script/
│ ├── First&Messy.R # Original monolithic script (for comparison)
│ ├── make_dummy_redcap_input.R# Generate anonymized dummy REDCap data
│ └── run.R # End-to-end pipeline using src01–05
│
├── input/
│ └── dummy_redcap_input.xlsx # Fake data only (no real patient info)
│
├── output/
│ └── *.xlsx # Analysis-ready outputs
│
└── results/
└── qc/ # QC reports
...
⚠️ **Real clinical data should NEVER be pushed to GitHub.**
...
Outputs are intentionally identical to the original script —
...
If something looks verbose, that’s intentional 🙂
---
## 📋 Clinical Data Management context
This project aligns with real-world Clinical Data Management responsibilities, including:
- CRF lifecycle handling (import → clean → deliver)
...
---
## 📚 Related standards & knowledge areas
